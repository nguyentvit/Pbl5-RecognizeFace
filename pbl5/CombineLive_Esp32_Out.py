...
class Face_Recognizer:
    ...
    def clear_csv(self):
        ...
        pd.DataFrame(columns=columns).to_csv('dataRecognizeOut.csv', mode='w', header=True, index=False)

    def save_to_csv(self, data):
        self.clear_csv()
        columns = ["Status", "Id", "Time", "PersonName", "StatusRecognized", "Img_Path"]
        new_data = pd.DataFrame([data])
        if os.path.exists('dataRecognizeOut.csv'):
            new_data.to_csv('dataRecognizeOut.csv', mode='a', header=False, index=False)
        else:
            new_data.to_csv('dataRecognizeOut.csv', mode='w', header=columns, index=False)
    ...
    def process(self, stream):
        if self.get_face_database():
            while stream.isOpened():
                        ...
                        if self.current_frame_face_cnt == 0:
                            ...
                            for id in self.current_frame_face_name_list:
                                if id != 'unknown' and len(self.current_frame_face_name_list) > 0:
                                    ...
                                    data = {"Status": False, "Id": id, "Time": time_saved, "PersonName": name,
                                            "StatusRecognized": True, "Img_Path": time_saved}
                ...
                self.update_fps()
                cv2.namedWindow("cameraOut", 1)
                cv2.imshow("cameraOut", img_rd)
